    ...
    const run = async () => {
      try {
        ...
        if (response.status === 202) {
          setPayload(data);
          setTimeout(() => setAttempt((value) => value + 1), 1500);
          return;
        }
      ...
      <section className="mx-auto max-w-5xl">
        ...
        {payload?.order && (
          <div className="mt-8 grid gap-4 rounded-[28px] border border-white/10 bg-white/[0.03] p-6 sm:grid-cols-3">
            <div>
              <p className="text-[10px] uppercase tracking-[0.25em] text-stone-500">Payment</p>
              <p className="mt-2 text-lg font-semibold text-white">{payload.order.payment_status || "paid"}</p>
            </div>
            <div>
              <p className="text-[10px] uppercase tracking-[0.25em] text-stone-500">Fulfillment</p>
              <p className="mt-2 text-lg font-semibold text-white">{(payload.order.fulfillment_status || "pending").replaceAll("_", " ")}</p>
            </div>
            <div>
              <p className="text-[10px] uppercase tracking-[0.25em] text-stone-500">Total</p>
              <p className="mt-2 text-lg font-semibold text-[#f4c66a]">{money(payload.order.amount_total, payload.order.currency)}</p>
            </div>
          </div>
        )}
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    <div>
                      ...
                      <p className="mt-2 text-xs uppercase tracking-[0.2em] text-stone-500">
                        {[item.color, item.size, item.format].filter(Boolean).join(" · ") || "Standard edition"}
                      </p>
                    </div>
                  ...
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      ...
                      <p className="mt-2 text-xs uppercase tracking-[0.18em] text-stone-500">
                        {entitlement.ready ? `${entitlement.downloads_remaining} secure download${entitlement.downloads_remaining === 1 ? "" : "s"} remaining` : entitlement.state.replaceAll("_", " ")}
                      </p>
                    </div>
                    <button
                      type="button"
                      disabled={!entitlement.ready}
                      onClick={() => download(entitlement.entitlement_id)}
                      className={`rounded-full px-6 py-3 text-xs font-black uppercase tracking-[0.18em] ${entitlement.ready ? "bg-[#f4c66a] text-black hover:bg-[#ffe09a]" : "cursor-not-allowed bg-stone-800 text-stone-500"}`}
                    >
                      {entitlement.ready ? "Secure Download" : "Master Pending"}
                    </button>
                  </div>
                </article>
              ))}
