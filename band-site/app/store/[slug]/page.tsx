...
function localProduct(slug: string): Product | null {
  ...
  return {
    ...
    checkout_enabled: item.checkoutEnabled,
...
export default function DirectMerchProductPage() {
  ...
  const [product, setProduct] = useState<Product | null>(() => localProduct(slug));
  const [loading, setLoading] = useState(() => !localProduct(slug));
  ...
  useEffect(() => {
    const canonical = localProduct(slug);
    if (canonical) {
  ...
  const canBuy = Boolean(product && product.checkout_enabled && product.sale_mode !== "sold_out" && product.sale_mode !== "coming_soon" && (!needsSize || selectedSize) && (!needsColor || selectedColor));
  ...
  async function buyNow() {
    ...
    try {
      const response = await fetch("/api/checkout", {
        ...
        body: JSON.stringify({ items: [{ id: product.id, quantity: 1, size: selectedSize || undefined, color: selectedColor || undefined }], returnPath: `/store/${product.slug}` })
      });
    ...
    <main className="min-h-screen bg-[#050403] px-5 py-12 text-white md:py-20">
      <div className="mx-auto max-w-6xl">
        <div className="mb-8 flex items-center justify-between gap-4">
          <Link href="/store" className="text-xs font-bold uppercase tracking-[0.22em] text-stone-400 hover:text-[#f4c66a]">← Store</Link>
          <span className="text-[10px] font-bold uppercase tracking-[0.28em] text-[#f4c66a]">Official KAM DRIDI merch</span>
        </div>
        {loading && <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-8 text-stone-300">Loading product…</div>}
        {!loading && error && !product && (
          <div className="rounded-3xl border border-red-500/25 bg-red-500/[0.06] p-8">
            <h1 className="text-2xl font-bold">Product unavailable</h1><p className="mt-3 text-stone-300">{error}</p>
            <Link href="/store" className="mt-6 inline-flex rounded-full bg-[#f4c66a] px-6 py-3 text-xs font-bold uppercase tracking-[0.2em] text-black">Back to store</Link>
          </div>
        )}
        {product && (
          <div className="grid gap-10 lg:grid-cols-2 lg:items-start">
            ...
            <section>
              ...
              {needsColor && <div className="mt-8"><p className="mb-3 text-xs font-bold uppercase tracking-[0.22em] text-stone-400">Choose color</p><div className="flex flex-wrap gap-2">{colors.map((color) => <button key={color} type="button" onClick={() => setSelectedColor(color)} className={`rounded-full border px-4 py-2 text-xs font-bold uppercase tracking-[0.16em] ${selectedColor === color ? "border-[#f4c66a] bg-[#f4c66a] text-black" : "border-white/15 bg-white/[0.03] text-stone-300"}`}>{color}</button>)}</div></div>}
              {needsSize && <div className="mt-8"><p className="mb-3 text-xs font-bold uppercase tracking-[0.22em] text-stone-400">Choose size</p><div className="flex flex-wrap gap-2">{sizes.map((size) => <button key={size} type="button" onClick={() => setSelectedSize(size)} className={`min-w-12 rounded-full border px-4 py-2 text-xs font-bold uppercase tracking-[0.16em] ${selectedSize === size ? "border-[#f4c66a] bg-[#f4c66a] text-black" : "border-white/15 bg-white/[0.03] text-stone-300"}`}>{size}</button>)}</div></div>}
              {error && <p className="mt-6 rounded-xl border border-red-500/25 bg-red-500/[0.06] p-4 text-sm text-red-200">{error}</p>}
              <button type="button" onClick={buyNow} disabled={!canBuy || checkoutLoading} className="mt-9 w-full rounded-full bg-[#f4c66a] px-6 py-5 text-sm font-black uppercase tracking-[0.22em] text-black transition hover:bg-[#ffd989] disabled:cursor-not-allowed disabled:opacity-40">{checkoutLoading ? "Opening secure checkout…" : "Buy now — secure Stripe checkout"}</button>
              <p className="mt-3 text-center text-xs text-stone-500">{(needsSize && !selectedSize) || (needsColor && !selectedColor) ? "Choose the required variant above, then checkout opens directly in Stripe." : "One product. One checkout. No store search required."}</p>
            </section>
          </div>
        )}
      </div>
